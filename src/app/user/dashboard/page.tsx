...
import {
  CheckCircle2, FileText, Loader2, RefreshCcw, MapPin, Sparkles, Home, MapPinned,
} from "lucide-react";

// Tidak perlu import Header atau Footer di sini
...
export default function DashboardPage() {
  const [applications] = useState<Application[]>([
    ...
    { id: 2, cluster: "Rumah Klasik Menteng", city: "Jakarta Pusat", status: "Dalam Review", loanAmount: 25000000000, date: "18 Juli 2025", image: "/rumah-2.jpg" },
  ]);
  ...
  return (
    // Halaman sekarang dimulai langsung dari kontennya, tanpa div pembungkus atau <header>
    <>
      {/* DASHBOARD HEADER */}
      <section className="py-12 bg-[#E0F7F5]">
      ...
      <section className="max-w-7xl mx-auto w-full px-4 py-10">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-xl md:text-2xl font-bold text-gray-800">Aplikasi KPR Saya</h2>
        </div>
        ...
        <div className="flex flex-col gap-6">
          {applications.map(app => {
            ...
            return (
              <div key={app.id} className="group bg-white rounded-2xl overflow-hidden border border-gray-200 shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
                <div className="grid md:grid-cols-[280px_1fr]">
                  <div className="relative h-48 md:h-full">
                    <Image src={app.image} alt={app.cluster} fill className="object-cover" sizes="(min-width: 768px) 280px, 100vw" />
                  ...
                  <div className="p-6 flex flex-col gap-4 bg-white">
                    ...
                    <div className="flex items-center justify-end pt-2 mt-auto">
                      <button className="rounded-xl px-4 py-2 text-sm font-semibold text-white shadow-md hover:shadow-lg transition-transform hover:scale-105" style={{ background: `linear-gradient(90deg, ${COLORS.teal}, ${COLORS.lime})` }}>Lihat Detail</button>
                    </div>
                  </div>
                </div>
              </div>
...
// ... Sisa komponen (SummaryCard, InfoItem) tetap sama

function SummaryCard({ icon, title, value, sub, accent, isMoney = false }: {
  icon: ReactNode; title: string; value: string; sub?: string; accent: string; isMoney?: boolean;
}) {
  return (
