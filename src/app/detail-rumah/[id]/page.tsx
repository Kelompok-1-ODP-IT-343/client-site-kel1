...
import { ChevronRight, MapPin, Home, CreditCard, FileText, BadgeInfo, Landmark, Compass, MessageCircle } from "lucide-react";
  ...
  return (
    <main className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-3xl sm:text-4xl font-bold text-gray-800">{detail.title}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 flex flex-col gap-8">
          ...
          <Card title="Deskripsi & Spesifikasi" icon={<FileText />}>
            ...
            <h3 className="text-xl font-bold text-gray-800 mb-3">Detail Properti</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 text-sm mb-6">
              <InfoItem label="Tipe Properti" value={fmtNum(detail.property_type)} icon={<BadgeInfo size={16} />} />
              <InfoItem label="Kode" value={fmtNum(detail.property_code)} icon={<FileText size={16} />} />
            </div>

            <h3 className="text-xl font-bold text-gray-800 mb-3">Lokasi</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 text-sm mb-6">
              <InfoItem label="Kota" value={fmtNum(detail.city)} icon={<Landmark size={16} />} />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-2 text-sm mb-6">
              {detail.features.length > 0 ? (
                detail.features.map((f, idx) => (
                  <InfoItem key={idx} label={f.featureName} value={f.featureValue} icon={<BadgeInfo size={16} />} />
                ))
              ) : (
            ...
            <h3 className="text-xl font-bold text-gray-800 mb-3">Lokasi Terdekat</h3>
            <div className="grid grid-cols-1 gap-y-2 text-sm mb-6">
              {detail.locations.length > 0 ? (
                detail.locations.map((l, idx) => (
                  <InfoItem key={idx} label={l.poiName} value={`${l.distanceKm} km`} icon={<Compass size={16} />} />
                ))
              ) : (
                <p className="text-gray-500">-</p>
              )}
